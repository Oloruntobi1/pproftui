...
// formatDeltaWithRatio formats a delta value with ratio information for enhanced diff display.
// Shows ratios for significant changes (>1.1x) and percentages for smaller changes.
// Handles "new" and "removed" function indicators.
func formatDeltaWithRatio(before, after int64, unit string, s *Styles) string {
	delta := after - before
	ratio := calculateRatio(before, after)

	// Handle special cases first
	if math.IsInf(ratio, 1) {
		// New function
		formattedVal := formatValue(after, unit)
		return s.DiffPositive.Render(fmt.Sprintf("+%s (new)", formattedVal))
	}
	if ratio == 0.0 {
		// Removed function
		formattedVal := formatValue(before, unit)
